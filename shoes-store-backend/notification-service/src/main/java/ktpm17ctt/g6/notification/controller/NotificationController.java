...
public class NotificationController {
    ...
    public ResponseEntity<String> sendEmail(@Valid @RequestBody EmailRequest emailRequest) {
        try {
            log.info("Received request to send email to {}", emailRequest.getTo().replaceAll("(?<=.{3}).(?=.*@)", "*"));
            emailService.sendEmail(emailRequest);
            log.info("Email sent successfully to {}", emailRequest.getTo().replaceAll("(?<=.{3}).(?=.*@)", "*"));
            return ResponseEntity.ok("Email đã được gửi thành công!");
        } catch (EmailSendingException e) {
            log.error("Failed to send email to {}: {}", emailRequest.getTo().replaceAll("(?<=.{3}).(?=.*@)", "*"), e.getMessage());
